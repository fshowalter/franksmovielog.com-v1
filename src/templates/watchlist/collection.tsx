...
export default function WatchlistDirectorTemplate({
  ...
  if (!data.collection.avatar) {
    throw Error(`No avatar found for ${data.collection.name}.`);
  }
      ...
      <main className={containerCss}>
        <div className={leftCss}>
          <FilterPageHeader
            ...
            avatar={data.collection.avatar.childImageSharp.gatsbyImageData}
            alt={`An image of ${data.collection.name}`}
            heading={data.collection.name}
            tagline={`Collection of ${data.movie.nodes.length} watchlist movies.`}
          />
          <Fieldset className={filtersCss}>
            <legend>Filter &amp; Sort</legend>
            <Label htmlFor="watchlist-collection-title-input">
              Title
              <DebouncedInput
                id="watchlist-collection-title-input"
            ...
            <RangeInput
              label="Release Year"
              id="watchlist-collection-release-year-input"
              min={state.minYear}
              max={state.maxYear}
              onChange={(values) =>
            ...
            />
            <Label htmlFor="watchlist-collection-sort-input">
              Order By
              <SelectInput
                value={state.sortType}
                id="watchlist-collection-sort-input"
                onChange={(e) =>
...
export const pageQuery = graphql`
  query ($name: String!) {
    collection: watchlistEntitiesJson(
    ...
    movie: allWatchlistMoviesJson(
      ...
      filter: { collection_names: { in: [$name] } }
    ) {
      nodes {
        imdbId: imdb_id
        title
