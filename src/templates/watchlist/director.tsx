...
export default function WatchlistDirectorTemplate({
  ...
  if (!data.director.avatar) {
    throw Error(`No avatar found for ${data.director.name}.`);
  }
      ...
      <main className={containerCss}>
        <div className={leftCss}>
          <FilterPageHeader
            ...
            avatar={data.director.avatar.childImageSharp.gatsbyImageData}
            alt={`An image of ${data.director.name}`}
            heading={data.director.name}
            tagline={`Director of ${data.movie.nodes.length} watchlist movies.`}
          />
...
export const pageQuery = graphql`
  query ($imdbId: String!) {
    director: watchlistEntitiesJson(
    ...
    movie: allWatchlistMoviesJson(
      ...
      filter: { director_imdb_ids: { in: [$imdbId] } }
    ) {
      nodes {
        imdbId: imdb_id
        title
